...
```{r}
df <- as.data.frame(BigDayTornadoes) %>%
  mutate(maxEF = as.integer(mag)) %>%
  select(groupNumber, maxEF, dist, ED, maxEF, Year)
...
```{r}
pr_k <- as.vector(table(df$maxEF) / nrow(df))
...
```{r}
df$logEDs <- scale(log(df$ED))
df$Yrs <- scale(df$Year)
df$dists <- scale(df$dist)
```
...
m1.2 <- map(
  alist(
    maxEF1 ~ dordlogit( phi, c(a0, a1, a2, a3, a4) ),
    phi <- bYr * Yrs + bDis * dists,
    bYr ~ dnorm(0, 10),
    bDis ~ dnorm(0, 10),
    c(a0, a1, a2, a3, a4) <- dnorm(0, 10)
  ) ,
  data = df ,
  start = list(a0 = -1, a1 = 0, a2 = 1, a3 = 2, a4 = 3, bYr = 0, bDis = 0)
)
```
...
Set year equal to 1994 (`Yrs = -2`) and distance to nearest city to something large (6 scaled).
```{r}
Yrs <- 0
dists <- -1.3
predictions.df <- post.df %>%
  mutate(City = "Near",
         phi = bYr * Yrs + bDis * dists,
         pk0 = pordlogit(1, a = a0, phi = phi),
         pk1 = pordlogit(2, a = a1, phi = phi),
         pk2 = pordlogit(3, a = a2, phi = phi),
         pk3 = pordlogit(4, a = a3, phi = phi),
         pk4 = pordlogit(5, a = a4, phi = phi),
         pEF0 = pk0,
         pEF1 = pk1 - pk0,
         pEF2 = pk2 - pk1,
         pEF3 = pk3 - pk2,
         pEF4 = pk4 - pk3,
         pEF5 = 1 - pk4,
         check = pEF0 + pEF1 + pEF2 + pEF3 + pEF4 + pEF5)

dists <- 6
predictions2.df <- post.df %>%
  mutate(City = "Far",
         phi = bYr * Yrs + bDis * dists,
         pk0 = pordlogit(1, a = a0, phi = phi),
         pk1 = pordlogit(2, a = a1, phi = phi),
...
plotPreds.df <- rbind(predictions.df, predictions2.df)

ggplot(plotPreds.df, aes(x = pEF0, color = City)) +
  geom_density()
...
ggplot(plotPreds.df, aes(x = pEF5, color = Time)) +
  geom_density()

ggplot(plotPreds.df, aes(x = Time, y = pEF1)) +
  geom_line() +
  geom_line(aes(x = Time, y = pEF2), color = "green") +
  scale_y_continuous(limits = c(0, 1))
...
for ( s in 1:10 ) {
    p <- post.df[s,]
    ak <- as.numeric(p[1:5])
    phi <- p$bYr * Yrs + p$bDis * dists
    pk <- pordlogit( 1:5 , a = ak , phi = phi )
    tpk <- rbind(tpk, pk)
}

tpk.df <- as.data.frame(tpk)
tpk.df$X <- rep(c(0, 1), times = 10)
tpk.df$S <- rep(1:10, each = 2)
names(tpk.df) <- c("EF0", "EF1", "EF2", "EF3", "EF4", "X")

ggplot(tpk.df, aes(x = X, y = EF0, group = S)) +
  geom_line() +
  geom_line(aes(y = EF1), color = "green") +
  geom_line(aes(y = EF2), color = "orange") +
  scale_y_continuous(limits = c(0, 1))
...
plot( 1 , 1 , type="n" , xlab="time" , ylab="probability" ,
    xlim=c(-2,1.5) , ylim=c(0,1) , xaxp=c(0,1,1) , yaxp=c(0,1,2) )
...
for ( s in 1:100 ) {
    ...
    pk <- pordlogit( 1:5 , a = ak , phi = phi )
    for ( i in 1:5 )
        lines( Yrs , pk[ ,i] , col=col.alpha(rangi2,0.1) )
}
