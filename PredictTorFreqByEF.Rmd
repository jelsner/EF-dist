...
The science of forecasting where an outbreak of severe convective weather might occur days in advance is well developed. For example, considerable skill exists in outlining what regions of the United States have the largest risk of severe weather on any given day. But more work is needed to anticipate specific characteristics of the outbreaks. Here we develop a statistical model that can be used to predict the probability distribution of tornado counts by EF rating on days expected to produce an outbreak. The model is fit using demographic data to condition on the fact that damage ratings are related to the number (and type) of potential targets. The model quantifies the importance of demographic information relative to information from environmental factors known to influence tornadoes, including CAPE and bulk shear. The model controls for distance to nearest city to quantify the percent increase in the chance of at least one tornado rated EF3 or higher for a unit increase in CAPE and bulk shear. The model is an ordered logistic regression and the coefficients are determined using the method of Hamiltonian Monte Carlo with the Stan language. Stan code is generated from R through the 'rethinking' package. The flexibility of this approach makes it easy to adjust the model to forecast other outbreak characteristics and to include domain-specific knowledge.
...
for(i in seq_len(nrow(BigDayTornadoes))){
  dist[i] <- min(st_distance(BigDayTornadoes[i, ], C.sf))
}
...
closest.df <- numeric()
 for(i in seq_len(nrow(BigDayTornadoes))){
     closest.df <- rbind(closest.df, as.data.frame(C.sf[which.min(st_distance(C.sf, BigDayTornadoes[i, ])), ]))
 }
...
df2 <- df %>%
  ...
  select(ID, maxEF, dist, maxEF, Year, maxCAPE, maxHLCY, maxBS, minCIN)
...
table(All_Tornadoes$mag)/dim(All_Tornadoes)[1]
```

The histogram of tornadoes by EF rating on big outbreak days is as expected with the vast majority rated EF0 or EF1 and far fewer rated EF4 or EF5. But relative to all tornadoes the distribution favors higher ratings. For example 4.5\% of big outbreak day tornadoes are rated EF3 compared with 2.3\% of all tornadoes. And .1\% of big outbreak day tornadoes are rated EF5 compared with .04\% of all tornadoes. We re-describe this histogram on the log-cumulative-odds scale. This means constructing the odds of a cumulative probability and then taking a logarithm. The logit is log-odds. The cumulative logit is log-cumulative-odds. Both constrain the probabilities to the 0/1 interval. 
...
ggplot(plot.df, aes(x = maxEF, y = cum_pr_k_all)) +
  ...
  theme_minimal()
```
**Cumulative proportion of tornadoes by maximum EF rating for all tornadoes (gray) and for tornadoes occurring on big days in big outbreaks (black)**
