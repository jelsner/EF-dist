...
formula <- maxEF1 ~ dists + CAPEs + HLCYs + BSs + CINs  + (1|id) + cor_bsts(~ Yr)
...
post.df <- posterior_samples(post)
```
...
```{r, eval=FALSE}
kfold1 <- kfold(post, folds = "stratified", group = "id")
kfold2 <- kfold(post, group = "id", chains = 1) 
```

```{r}
library(bayestestR)
equivalence_test(post)
```
...
```{r}
gg <- marginal_effects(post, categorical = TRUE)
ggBS <- gg[[5]]
