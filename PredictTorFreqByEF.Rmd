...
A single group can be across more than one convective day. And there can be more than one group on a convective day. ID provides a unique label for the date and group number.
```{r}
dim(BigDays.sfdfT)
...
A map of one convective-day group used in this study.
```{r}
...
tm_shape(sts) +
  ...
  tm_text("name", size = .6, remove.overlap = TRUE) +
tm_shape(BDt) +
  tm_dots(size = .5) +
...
```
### Figure: Locations of tornadoes (origin) during one convective-day group used in this study.
...
( p0 <- ggplot(BigDays.sfdfT, aes(x = Month)) +
  geom_bar(fill = "gray70") +
...
ggplot(BigDays.sfdfT, aes(x = nT)) +
  ...
  theme_minimal() 
```
### Figure: Monthly frequency of convective-day groups for groups with at least ten tornadoes (1994--2017).
...
We begin with a histogram of maximum EF rating per tornado. We need the `BigDaysInLargeGroups.RData` file to get all tornadoes. When we load this the file `BigDays.sfdfT` is now only days within a large group.
```{r}
df2 <- df %>%
...
```
### Figure: Histogram of tornadoes by maximum EF rating. Only tornadoes occurring in big convective-day groups are considered (see text).
...
```
### Figure: Posterior median and highest posterior density intervals for the model's fixed effects.
...
#BSs <- seq(-3, 2.5, by = .5)
fitted <- df2 %>%
  data_grid(Yrs = 0, dists = -1.3, CAPEs = 0, HLCYs = 0, BSs = c(-3, -1, 0, 2.5), id = 30, mo = 5) %>%
...
tpk.df <- data.frame(X = rep(c(-3, -1, 0, 2.5), each = 10),
                     S = rep(1:10, times = 4),
                     EF0 = x$cs[x$.category == 1],
                     EF1 = x$cs[x$.category == 2],
                     EF2 = x$cs[x$.category == 3],
                     EF3 = x$cs[x$.category == 4],
                     EF4 = x$cs[x$.category == 5])
...
ggplot(aes(x = BSs, y = cs, group = .draw, color = .category)) +
  geom_point()
...
```{r}
predicted <- df2 %>%
  data_grid(Yrs = 0, dists = -1.3, CAPEs = 0, HLCYs = 0, BSs = c(-3, 0, 2.5), id = 30, mo = 5) %>%
  add_predicted_draws(fit1) 

dat <- df2[df2$id == 30,] 
table(dat$maxEF)/nrow(dat)
...
Examine potential interaction between HLCY and CAPE.
```{r}
...
```
There is no evidence to support an interaction between BS and CAPE or HLCY and CAPE or Yr and dist.
...
Here we examine whether the effect of CAPE different across different EF ratings.
```{r, eval=FALSE}
...
Add some specific numbers. Make predictions with the `predict()` function specifying values. `id = 5` corresponds to the outbreak delineated on the map in Figure 1.
```{r}
predict(fit1, newdata = data.frame(Yrs = 0, dists = -1.3, CAPEs = 0, HLCYs= 0, BSs = 2.4, mo = 1:12, id = 5))
predict(fit6, newdata = data.frame(Yrs = 0, dists = -1.3, CAPEs = 0, HLCYs= 0, BSs = 2.4, mo = 1:12, id = 5))
