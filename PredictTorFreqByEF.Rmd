...
It should be noted that because the statistical model was fit to data exclusively from large clusters, the probability output is only valid when an outbreak occurs.
...
ggplot(ggCAPE, aes(x = CAPE, y = estimate__, col = cats__)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = cats__), 
              alpha = .7, col = NA) +
  scale_color_brewer(name = "EF", label = c("0", "1", "2", "3", "4", "5"), palette = "Blues") +
