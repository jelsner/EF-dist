---
title: "Tornado damage rating probabilities estimated with cumulative logistic regression"
author: "James Elsner"
date: "2/14/2019"
...
Tornado damage rating probabilities estimated with cumulative logistic regression
...
#load("~/Desktop/Projects/EF-dist/Climatology.RData")
st_crs(Torn.sfT)
...
A single group can be across more than one calendar day. And there can be more than one group on a convective day. ID provides a unique label for the date and group number.
```{r}
dim(BigDays.sfdfT)
...
tm_shape(sts) +
  ...
  tm_text("name", size = .6, remove.overlap = TRUE, col = "gray80") +
tm_shape(BD) +
...
tm_layout(title = "April 26, 1994", title.bg.color = "white")
```
...
Given a group of at least ten tornadoes within a single convective 24-hour period (12 UTC to 12 UTC), we extract environmental variables from the North American Regional Reanalysis (NARR) obtained from the the National Center for Atmospheric Research (NCAR). Variables are available on a 32.4 km grid and represent of a blend of model and observational sounding data.
...
get_prior(formulaA, data = df2, family = family)
