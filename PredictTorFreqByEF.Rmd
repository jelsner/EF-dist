...
The study aims: (1) to show the feasibility of forecasting a particular outbreak characteristic, and (2) to demonstrate a flexible modeling framework that can be used to forecast other characteristics.
...
Load the data. This study uses the outbreak data collated in the tor-cluster GitHub project. The methodology for determining the big outbreak days is detailed in Schroder and Elsner (2019). Briefly, the method groups space-time distances between tornado genesis over the period 1994 through 2017, inclusive. If two tornadoes occur close in space and time then they are grouped together. Grouping stops when the the space-time distance threshold exceeds 50K meter-seconds.
```{r}
